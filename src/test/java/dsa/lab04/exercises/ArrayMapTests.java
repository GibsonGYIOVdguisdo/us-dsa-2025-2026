...
import dsa.lab04.data.MapItemSourceData;
...
public class ArrayMapTests
{
  ...
  static final ParameterResolver classResolver =
    TestClassUtils.resolver(ArrayMap.class);
        ...
        .flatReplace((itemss) ->
          itemss.flatReplace((items) ->
              items.replace((item, index) -> new Object[]{
                TestClassUtils.construct(arrayMapClass, items),
                item.key(),
                index}))
            .quadratic()
            .limit())
      ...
      Class<?> arrayMapClass)
    {
      return Source.from(
        ...
        .flatReplace((itemss) ->
          itemss.flatReplace((items) ->
              items.replace((item, index) -> new Object[]{
                TestClassUtils.construct(arrayMapClass, items.skipIndex(index)),
                item.key()}))
            .quadratic()
            .limit())
        .replace((arguments) ->
