...
and `remove(index)`. That's your job! Delete the line:
```java
// TODO: Implement StaticArray.insert(int index, Item item)
```
and replace it with code that correctly implements those two methods. Check
...
contains a subclass, `Node<Item>`, which implements the
...
or
```java
System.out.println(myList.toDebugString());
```
