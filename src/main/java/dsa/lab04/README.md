...
**only if** the class `Item` implements the `Comparable<Item>` interface, 
but we'll explain what that means in next week's lab.
...
map by using the `toString()` and `toDebugString()` methods that we implemented
for you. For instance, try to type:
```java
...
System.out.println(myArrayMap);
```
It should print the following (possibly with one or more different Object IDs):
```
dsa.lab04.solutions.ArrayMap (class)
Object ID: 404b9385
'-- .items
    '-- dsa.lab03.solutions.DynamicArray (class)
        Object ID: 682a0b20
        |-- .items
